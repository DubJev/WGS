...
setup_file() {
    ...
    test_dir=$PWD
    test_dir_mod=$PWD/tests/haplotyper
...
@test "TestHaplotyperModule" {
    run $HOME/nextflow run $test_dir/modules/haplotyper/haplotyper.nf -c $test_dir/configs/haplotyper/haplotyper.config --hcInputs=$test_input --verOutputs=$outdir --hcOutputs=$outdir
    [ "$status" -eq 0 ]
