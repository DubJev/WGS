...
setup_file() {
    ...
    test_dir=$PWD
    test_dir_mod=$PWD/tests/samtools
...
@test "TestSamtoolsModule" {
    run $HOME/nextflow run $test_dir/modules/samtools/samtools.nf -c $test_dir/configs/samtools/samtools.config --bams=$test_input --viewOutputs=$outdir
    [ "$status" -eq 0 ]
    ...
    do
        ...
        do
            ...
            run diff $file ${expected_outdir}/${name}
            [ "${status}" -eq 0 ]
        done
    done
